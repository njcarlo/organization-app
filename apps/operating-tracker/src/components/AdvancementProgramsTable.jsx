...
/**
 * Revenue Generating Programs — editable table embedded in the Advancement
 * "Enter Data" page, and rendered read-only (no header/cell editing, no
 * drag-reorder, no add affordances) inside the Advancement board Report.
...
const AdvancementProgramsTable = forwardRef(function AdvancementProgramsTable({ readOnly = false, bare = false }, ref) {
  ...
  const startEditCell = (row, col) => {
    if (readOnly || col.builtin) return
  ...
  const renderCell = (row, col) => {
    ...
    // custom column
    const isEditing = !readOnly && editingCell?.rowId === row.id && editingCell?.colId === col.id
    if (isEditing) {
      return (
        <input
          autoFocus
  ...
  return (
    <div className="print:text-black">
      ...
      <Wrapper className={wrapperClass}>
        {!bare && (
          <div className="flex items-center justify-between gap-3 border-b border-hae-line px-4 py-3 print:px-2 print:py-1">
            <div>
              <h2 className="font-display text-lg text-hae-ink print:text-[9px]">Revenue Generating Programs</h2>
              <p className="text-xs text-hae-slate print:hidden">
                {readOnly
                  ? 'Financial impact — click a program for its full financial report.'
                  : 'Financial impact — click a program for its full financial report. Click a column header to rename it, drag it to reorder.'}
              </p>
            </div>
            {!readOnly && (
