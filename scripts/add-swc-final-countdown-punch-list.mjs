#!/usr/bin/env node
/**
 * One-time content load: add a "Final Countdown Punch List" subcategory
 * (a new `customSectionItems` doc) under the existing "Startup World Cup
 * 2026" `customSections` doc, with one `projects` doc per day (Sept 14-24,
 * 2026) and one `tasks` doc per punch-list checklist item under that day,
 * mirroring the shapes TaskTable.jsx / CategoryProgramPage.jsx write when a
 * user adds a project/task by hand (see promote-startup-world-cup-category.mjs
 * for the sibling migration this pattern is copied from).
 *
...
async function main() {
  ...
  if (!section) {
    throw new Error(`No customSections doc found with label "${SECTION_LABEL}" — has it been renamed?`)
  }
  console.log(`Found customSections/${section.id} "${SECTION_LABEL}".`)

  const existingItem = items.find((i) => i.fields.sectionId === section.id && i.fields.name === ITEM_NAME)
  if (existingItem) {
    throw new Error(
      `customSectionItems/${existingItem.id} "${ITEM_NAME}" already exists under this section — remove it first if you want to re-run this script.`
    )
  }
  ...
  const writes = [
    createWrite('customSectionItems', itemId, {
      name: ITEM_NAME,
      lead: [],
  ...
  console.log(`Will create customSectionItems/${itemId} "${ITEM_NAME}" under customSections/${section.id}.`)

  let totalTasks = 0
  DAYS.forEach((day, dayIndex) => {
    const projectId = randomId()
    writes.push(
      createWrite('projects', projectId, {
        name: day.name,
        lead: [],
        promise: '',
        health: 'not-started',
    ...
    console.log(`Will create projects/${projectId} "${day.name}" (targetDate ${day.date}, order ${dayIndex}).`)

    day.tasks.forEach((taskName, taskIndex) => {
      const taskId = randomId()
      writes.push(
        createWrite('tasks', taskId, {
          name: taskName,
          owner: [],
          dueDate: day.date,
          status: 'Not Started',
          ...
          order: taskIndex,
          projectId,
          projectName: day.name,
          programId: itemId,
          programName: ITEM_NAME,
          createdAt: now,
        })
      )
      totalTasks += 1
    })
    console.log(`  + ${day.tasks.length} task(s).`)
  })

  console.log(`\nTotal: 1 subcategory, ${DAYS.length} day-project(s), ${totalTasks} task(s).`)
